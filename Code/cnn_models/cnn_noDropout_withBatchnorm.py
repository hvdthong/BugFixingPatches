...
class CNNPatchWithoutDropOutWithBatchNorm(object):
    """
    ...
    def pool_outputs(self, embedded_chars_expanded, W, b, max_length, filter_size):
        ...
        # conv_bn = tf.contrib.layers.batch_norm(conv, center=True, scale=True, is_training=self.phase)
        conv_bn = tf.contrib.layers.batch_norm(conv, center=False, scale=False, is_training=self.phase)
    ...
    def __init__(
        ...
        # Embedding layer
        with tf.device('/cpu:10'), tf.name_scope("embedding"):
        ...
        with tf.name_scope("output"):
            ...
            self.score_left = tf.nn.xw_plus_b(h_fusion_left, W, b, name="score_left")
            self.score_right = tf.nn.xw_plus_b(h_fusion_right, W, b, name="score_right")
