...
class CNNPatchBatchNorm(object):
    """
    ...
    def h_pool(self, num_filters_total, pooled_outputs):
        h_pool_ = tf.reshape(tf.concat(pooled_outputs, len(pooled_outputs)), [-1, num_filters_total])
        return h_pool_

    def h_fusion(self, W, h_pool_text, h_pool_code):
        transform = tf.matmul(h_pool_text, W)
        return tf.multiply(transform, h_pool_code)
    ...
    def __init__(
        ...
        with tf.name_scope("output"):
            W = tf.get_variable(
                "W_output",
                shape=[h_fusion_drop_left.get_shape()[1], 1],
                initializer=tf.contrib.layers.xavier_initializer())
